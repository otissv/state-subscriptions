...
describe('createStore', () => {
  ...
  it('should publish event', () => {
    ...
    const store = createStore(preloadState);
    const type = 'test';
    const action = () => ({ test: 'next state' });

    jest.spyOn(store, 'publish');
    jest.spyOn(store, 'emit');
    jest.spyOn(store, 'get');

    store.publish([type, action]);

    expect(store.state.test).toBe('next state');

    expect(store.publish).toBeCalledTimes(1);
    expect(store.publish).toBeCalledWith([type, action]);
    expect(store.publish).toReturnWith(store);

    expect(store.emit).toBeCalledTimes(1);
    expect(store.emit).toBeCalledWith('test', 'next state');
  ...
  it('should call onPublish when publishing event', () => {
    ...
    store.publish([type, { test: 'next state' }]);

    expect(callback).toBeCalledTimes(1);
    expect(callback).toBeCalledWith([type, { test: 'next state' }]);
  });
  ...
  it('should receive subscribed event', () => {
    ...
    expect(listener).toBeCalledWith(action.test);
  });
  ...
  it('should call onSubscribe when publishing event', () => {
    ...
    expect(callback).toBeCalledTimes(1);
    expect(callback).toBeCalledWith([type, action.test]);
  });
